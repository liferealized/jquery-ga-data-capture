	...
	$.fn.gaDataCapture = function() {
		...
		this.each(function(i) {
			...
			var $this = $(this),
				...
				$keyInput = $('<input type="hidden" name="keywords" />'),
				$typInput = $('<input type="hidden" name="searchType" />'),
				$engInput = $('<input type="hidden" name="searchEngine" />');
			...
			if (tagName === 'form' && umtzData) {
				...
				if ($this.find('[name=keywords]').length) {
					$this.find('[name=keywords]').val(unescape(umtzData.utmctr));
				} else {
					$keyInput.val(unescape(umtzData.utmctr));
					$this.prepend($keyInput);
				}
				
				if ($this.find('[name=searchType]').length) {
					$this.find('[name=searchType]').val(unescape(umtzData.utmcmd));
				} else {
					$typInput.val(unescape(umtzData.utmcmd));
					$this.prepend($typInput);
				}
				
				if ($this.find('[name=searchEngine]').length) {
					$this.find('[name=searchEngine]').val(unescape(umtzData.utmcsr));
				} else {
